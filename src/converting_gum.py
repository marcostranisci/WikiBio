...
def modify_gum_labels(a_list,ent):
    ...
    for line in a_list:
        ...
        if re.search('\({}\)'.format(ent),line[-1]):
            gum4entity.append((line[1],'ENTITY',line[-1]))
            prev = 0
            ...
            gum4entity.append((line[1],'0',line[-1]))
        else: gum4entity.append((line[1],'0',line[-1]))
    return gum4entity
...
def converting_gum_coref(folder):
    revised_corpus = list()
    for doc in glob.glob('{}/*.conll'.format(folder)):
        with open(doc) as f:
            lines = f.readlines()
