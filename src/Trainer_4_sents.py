...
class Report():
    ...
    def make_report(self,values:Dict):
        if os.path.exists('../report/sent_detection_reports/{}'.format(self.file)):
            with open('../report/sent_detection_reports/{}'.format(self.file),mode='a') as f:
...
class Trainer():
    ...
    def do_train(self):
        ...
        for epoch_num in range(self.EPOCHS):
            ...
            length_train = 0
            for batch in tqdm(self.train,total=len(self.train)):
                    ...
                    length_train+=len(train_label)


                    loss, logits = self.model(features=input_id, attention_mask=mask,labels=train_label)
                    predictions = logits.argmax(dim=1)
                    acc = (predictions == train_label[0]).float().mean()

                    total_acc_train += acc
                    total_loss_train += loss.item()

                    precision,recall,fscore,support = precision_recall_fscore_support(train_label[0].cpu(),predictions.cpu(),average='macro',zero_division=0)
            ...
            total_fscore_val = 0
            length_eval = 0
            for batch in tqdm(self.eval,total=len(self.eval)):

                    val_label = batch['targets'].to(device)
                    mask = batch['attention_mask'].squeeze(1).to(device)
                    input_id = batch['features'].squeeze(1).to(device)
                    length_eval+=len(val_label)

                    with torch.no_grad():
                        loss, logits = self.model(features=input_id, attention_mask=mask,labels=val_label)
                        predictions = logits.argmax(dim=1)
                        acc = (predictions == val_label[0]).float().mean()

                        total_acc_val += acc
                        total_loss_val += loss.item()
                        precision,recall,fscore,support = precision_recall_fscore_support(val_label[0].cpu(),predictions.cpu(),average='macro',zero_division=0)
        ...
        total_fscore_test = 0
        length_test = 0
        for b in tqdm(self.test,total=len(self.test)):

                test_label = batch['targets'].to(device)
                mask = batch['attention_mask'].squeeze(1).to(device)
                input_id = batch['features'].squeeze(1).to(device)
                length_test+=len(test_label)

                with torch.no_grad():
                    loss, logits = self.model(features=input_id, attention_mask=mask,labels=test_label)
                    predictions = logits.argmax(dim=1)
                    acc = (predictions == test_label[0]).float().mean()

                    total_acc_test += acc
                    total_loss_test += loss.item()
                    precision,recall,fscore,support = precision_recall_fscore_support(test_label[0].cpu(),predictions.cpu(),average='macro',zero_division=0)
